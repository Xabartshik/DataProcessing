...
# === Две сферы по тройкам измерений (1-3) и (4-6) для двух решений кластеризации ===
print("=== Сферы из исходных координат без проекции: (1-3) и (4-6) ===")
    ...
    try:
        ...
        for cid, region in enumerate(sv.regions):
            ...
            for j in range(len(verts)):
                ...
                b = 1 + ((j + 1) % len(verts))
                tri_i.append(0); tri_j.append(a); tri_k.append(b)
            # Небольшая полупрозрачность, а точки рисуем поверх, чтобы их было видно
            patches.append(go.Mesh3d(
                x=verts_all[:,0], y=verts_all[:,1], z=verts_all[:,2],
                i=tri_i, j=tri_j, k=tri_k,
                color=colors[cid % len(colors)], opacity=0.35,
                name=f"cluster {cid} region"
            ))
    except Exception as e:
        print(f"[WARN] SphericalVoronoi не построен: {e}")
...
def build_two_spheres_for_dims(tag, X, labels, centers, k, dims_triplets, title_prefix):
    for dims in dims_triplets:
        # Берем напрямую исходные координаты по указанным измерениям (индексация 0-based)
        X_sel = X[:, dims]                 # (n, 3)
        C_sel = centers[:, dims]           # (k, 3)
        # Нормировка на единичную сферу
        P = normalize_rows(X_sel)
        C = normalize_rows(C_sel)
        ...
        # Порядок: сначала сфера, затем области, затем точки (чтобы точки были видимы)
        ...
        fig.update_layout(
            ...
            margin=dict(l=0, r=0, t=0, b=0),
            title=f"{title_prefix}: сфера {dims[0]+1},{dims[1]+1},{dims[2]+1} (k={k})"
        )
        out = os.path.join(
            "figures",
            f"lab6_sphere_dims{dims[0]+1}{dims[1]+1}{dims[2]+1}_{tag}.html"
        )
...
# --- Сферы для вашей реализации KMeans ---
...
build_two_spheres_for_dims(
    ...
    title_prefix="Моё kmeans (без проекции)"
)
...
if 'best_solution_sklearn' in globals() and best_solution_sklearn is not None:
    build_two_spheres_for_dims(
        ...
        k=k_star_sk,
        dims_triplets=dims_list,
        title_prefix="sklearn.KMeans (без проекции)"
    )
