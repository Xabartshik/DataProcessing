...
#
# # переводим в numpy-массивы для удобства
# vals = [np.array(v) for v in vals]
#
# # оценка M_i и D_i по времени для каждой компоненты
# # D_i считается как 1/n * sum (x - M_i)^2  => ddof=0
# comp_means = [v.mean() for v in vals]              # или np.mean(v)
# comp_dispersions = [v.var(ddof=0) for v in vals]   # или np.var(v, ddof=0)
#
# # M(t), D(t) по всем компонентам
# Mt = []
# Dt = []
# for t_index in range(len(T)):
#     snapshot = np.array([vals[i - 1][t_index] for i in I])
#     m = snapshot.mean()
#     d = snapshot.var(ddof=0)
#     Mt.append(m)
#     Dt.append(d)
#
# print(f"\nОценки компонент:")
...
#     sample = vals[component_idx]  # уже numpy-массив
#     n = len(sample)
#
...
#     # здесь уже использовался numpy: mean/std с ddof=1
#     m_hat = sample.mean()
