...
# ========== УСЛОВНЫЕ СРЕДНИЕ ПО Y|X (НОВОЕ РАСПРЕДЕЛЕНИЕ) ==========
print("=== УСЛОВНЫЕ СРЕДНИЕ Y по каждому X (новое распределение) ===")
y_conditional_by_x = []  # список средних Y|X для каждого X_j в том же порядке
for i, x in enumerate(x_values):
    if nx[i] > 0:
        numerator = sum(y_values[j] * data[y_values[j]][i] for j in range(len(y_values)))
        y_bar_x = numerator / nx[i]
    else:
        y_bar_x = np.nan
    y_conditional_by_x.append(y_bar_x)
    print(f"x = {x}: ȳ|x = {y_bar_x:.1f} (nx = {nx[i]})")

print()

# Создадим «новое распределение»: для каждого наблюдения с данным x_j
# значение y заменяется на соответствующее условное среднее ȳ|x_j.
# Это соответствует переходу к условным средним, используемым далее «для всего».

# Полное разворачивание исходных точек для визуализации (как было в оригинале)
x_points_orig, y_points_orig = [], []
for i, x in enumerate(x_values):
    for j, y in enumerate(y_values):
        freq = data[y][i]
        for _ in range(freq):
            x_points_orig.append(x)
            y_points_orig.append(y)

# Разворачивание НОВОГО распределения по условным средним (используется дальше в расчётах)
x_points, y_points = [], []
for i, x in enumerate(x_values):
    if nx[i] > 0:
        for _ in range(nx[i]):
            x_points.append(x)
            y_points.append(y_conditional_by_x[i])

# Дополнительно сохраним компактное представление условных средних (для пузырьков)
cond_x = np.array(x_values, dtype=float)
cond_y = np.array(y_conditional_by_x, dtype=float)
cond_w = np.array(nx, dtype=float)

# ========== ШАГ 1: ПОСТРОЕНИЕ КОРРЕЛЯЦИОННОГО ПОЛЯ ==========
print("=== ШАГ 1: ПОСТРОЕНИЕ КОРРЕЛЯЦИОННОГО ПОЛЯ ===")
print(f"Общее количество точек нового распределения (по условным средним): {len(x_points)}")
...
# Средние значения (по новому распределению)
...
# Вариансы и стандартные отклонения (по новому распределению)
...
# Суммы для системы нормальных уравнений (по новому распределению)
sum_x  = sum(x_points)
sum_y  = sum(y_points)
...
# Уравнение регрессии через коэффициент корреляции (по новому распределению)
...
# Уравнение регрессии x на y (по новому распределению)
c1 = r * (s_x / s_y)
c0 = x_mean - c1 * y_mean
...
# Вычисляем предсказанные значения (по новому распределению)
...
# 1) Исходные точки (полное разворачивание), полупрозрачные
plt.scatter(x_points_orig, y_points_orig, alpha=0.25, s=30, c='gray', edgecolors='none', label='Исходные наблюдения')

# 2) Точки условные средние Y|X с размером по nx
for i, x in enumerate(x_values):
    if nx[i] > 0:
        size = 30 + 50 * (nx[i] / max(nx))
        plt.scatter([x], [y_conditional_by_x[i]], s=size, alpha=0.85, c='#1f77b4', edgecolors='black', linewidths=0.5)

# 3) Линия регрессии y на x (по условным средним)
x_line = np.linspace(min(x_points), max(x_points), 200)
y_line = a1 * x_line + a0
plt.plot(x_line, y_line, 'r-', linewidth=2.5, label=f'y = {a1:.3f}x {a0:+.0f}')

# 4) Линия регрессии x на y (по условным средним) — для сравнения
y_line2 = np.linspace(min(y_points), max(y_points), 200)
...
plt.title('Корреляционное поле и уравнения регрессии\n(использованы условные средние Y|X)', fontsize=14, fontweight='bold')
plt.legend(fontsize=10)
plt.grid(True, alpha=0.3)
plt.tight_layout()
fig_path = os.path.join("figures", "correlation_field_and_regression_conditional.png")
plt.savefig(fig_path, dpi=300, bbox_inches='tight')
plt.show()

print(f"График корреляционного поля с уравнениями регрессии построен и сохранен как '{fig_path}'")
print()
